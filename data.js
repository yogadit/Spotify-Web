var redirect_uri = "http://localhost/UASPTI/home.html"; // change this your value
...
function requestAuthorization() {
  ...
  url += "&redirect_uri=http%3A%2F%2Flocalhost%2FUASPTI%2F";
...
function fetchAccessToken(code) {
  ...
  body += "&code=" + code;
  body += "&redirect_uri=http%3A%2F%2Flocalhost%2FUASPTI%2Fhome.html";
...
function handleApiResponseSearch() {
  if (this.status == 200) {
    // var searchResult = this.responseText;
    // console.log(searchResult);
    var searchResult = JSON.parse(this.responseText);
    console.log(searchResult);
    removeAllItems("resultSearch");
    console.log(searchResult.tracks.items);
    searchResult.tracks.items.forEach((item, index) => addSearchResults(item, index));
    setTimeout(currentlyPlaying, 2000);
...
function addSearchResults(item, index) {
  let node = document.createElement("div");
  node.value = index;
  //   console.log(item);
  node.innerHTML = item.album.name + " (" + item.album.artists[0].name + ")";
  document.getElementById("resultSearch").appendChild(node);
}
function deviceId() {
...
function search() {
  ...
  var SearchResult = callApi("GET", SEARCH + url, null, handleApiResponseSearch);
  console.log(SearchResult);
}
